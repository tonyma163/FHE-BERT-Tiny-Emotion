...
Ctxt pooler(Ctxt input) {
    ...
    output = controller.eval_tanh_function(output, -1, 1, tanhScale, 131);
...
Ctxt encoder2(vector<Ctxt> inputs) {
    ...
    Ctxt scores_denominator = controller.eval_inverse_naive_2(scores_sum, 1.31, 68336.18, 1); // 68336.18 // 6514.89
    ...
    for (int i = 0; i < output.size(); i++) {
        output[i] = controller.eval_gelu_function(output[i], -1, 1, GELU_max_abs_value, 51);
...
vector<Ctxt> encoder1() {
    ...
    Ctxt scores_sum = controller.rotsum(scores, 128, 128);
    Ctxt scores_denominator = controller.eval_inverse_naive(scores_sum, 1.63, 4670.44); // 4670.44
    ...
    for (int i = 0; i < output.size(); i++) {
        output[i] = controller.eval_gelu_function(output[i], -1, 1, GELU_max_abs_value, 41);
        output[i] = controller.bootstrap(output[i]);
    }
