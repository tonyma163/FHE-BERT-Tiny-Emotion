...
vector<double> FHEController::decrypt_tovector6(const Ctxt &c, int slots) {
    Ptxt p;
    context->Decrypt(key_pair.secretKey, c, &p);
    p->SetSlots(slots);
    p->SetLength(slots);
    ...
    // extract 6 logits from the slots
    double logit0 = vec[0];
    double logit1 = vec[128];
    double logit2 = vec[256];
    double logit3 = vec[384];
    double logit4 = vec[512];
    double logit5 = vec[640];

    vector<double> res = {logit0, logit1, logit2, logit3, logit4, logit5};
...
Ctxt FHEController::eval_inverse_naive(const Ctxt &c, double min, double max) {
    return context->EvalChebyshevFunction([](double x) -> double { return 1 / x; }, c, min, max, 181); // 181 // 119
}

Ctxt FHEController::eval_inverse_naive_2(const Ctxt &c, double min, double max, double mult) {
    return context->EvalChebyshevFunction([mult](double x) -> double { return mult / x; }, c, min, max, 326); // 326 // 228
}
